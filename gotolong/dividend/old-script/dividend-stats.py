...
if len(sys.argv) < 6:
    print
    "usage: " + program_name + " <out_plain | out_csv> <sort_amount|sort_frequency|sort_name|name_only> <summary_yes|sumary_no> <debug_level : 1-4> <op-txn-hist.csv> ... "
    sys.exit(1)
...
# company_aliases_filename = PROJ_LOCAL_DATA_LOC + '/other-data/dividend-company-name-aliases.csv'
company_aliases_filename = PROJ_LOCAL_DATA_LOC + '/other-data/dividend-company-ticker-aliases.csv'
...
def load_aliases():
    with open(company_aliases_filename, 'r') as csvfile:
        ...
        for row in reader:
            ...
            if debug_level > 2:
                print
                'alias ', name_alias, 'ticker ', ticker
...
def resolve_real_company_name_db(company_name):
    ...
    for real_company_name in company_real_name_db:
        if company_name.find(real_company_name, 0) >= 0:
            if debug_level > 2:
                print
                'replaced from ', company_name, 'to ', real_company_name
...
def parse_line(line):
    ...
    if re.match('ACH/.*|CMS/.*', txn_remarks):
        ...
        if company_name in dividend_amount.keys():
            if debug_level > 1:
                print
                'dividend amount :', dividend_amount[company_name]
                print
...
if summary_type == "summary_yes" and sort_type == "sort_name":
    if out_type == "out_csv":
        print
        'total dividend amount:  ,', ' 0 ,', total_dividend
        print
        'total dividend entries: ,', ' 0 ,', len(companies)
        print
        'total companies count:  ,', ' 0 ,', len(comp_freq)
    else:
        print
        'total dividend amount:  ', ' 0 ', total_dividend
        print
...
if summary_type == "summary_yes" and sort_type == "sort_amount":
    if out_type == "out_csv":
        print
        'total dividend amount:  ,', total_dividend
        print
        'total dividend entries: ,', len(companies)
        print
        'total companies count:  ,', len(comp_freq)
    else:
        print
        'total dividend amount:  ', total_dividend
        print
