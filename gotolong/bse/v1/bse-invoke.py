...
def bse_load_dividend(row, security_name, ex_date, purpose):
    try:
        ...
        year = ex_date.split()[2]
        corp_act_security_year_dict[security_name, year] = 1
        corp_act_security_list.append(security_name)
    except NameError:
        if debug_level > 1:
            print('NameError', row)
...
def bse_load_row(data_type, row):
    ...
    if data_type == "d":
        bse_load_dividend(row, security_name, ex_date, purpose)
    if data_type == "b":
...
items = corp_act.items()

corp_act_security_year_dict[security_name, year] = 1
items = corp_act_security_list.items()
for year in
    print('security_name, dividend_amount, dividend_count')
