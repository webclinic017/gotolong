[surikuma@adc00oti ~/equity.d]$ cat idirect-stats.sh
#!/bin/sh

if test $# -lt 3
then
   echo "usage: $0 <file> <company | year | year_company> <all | tcs |  2017>"
   exit 1
fi
...
do
   ...
   do
       ...
       cat $FILE | grep $year | grep $SPEC | grep $month | awk -F','  ' BEGIN {tbuy = 0; tsale =0; company=""; } {tdate=$13; year=substr(tdate,8,4); month=substr(tdate,4,3); buysale=$4; company=company" "$1; if (buysale == "Buy") { tbuy += $5*$6; } else { tsale += $5*$6;} } END { if (tbuy != 0 || tsale != 0) {printf("%s %s txn# %3s sale %6s buy %6s net %6s\n", year, month, NR, int(tsale), int(tbuy), int(tbuy-tsale)); } }' 
   done
done
fi
...
do
   ...
   do
       ...
       cat $FILE | grep $year | grep $SPEC | grep $month | awk -F','  ' BEGIN {tbuy = 0; tsale =0; company=""; } {tdate=$13; year=substr(tdate,8,4); month=substr(tdate,4,3); buysale=$4; company=company" "$1; if (buysale == "Buy") { tbuy += $5*$6; } else { tsale += $5*$6;} } END { if (tbuy != 0 || tsale != 0) {printf("%s %s txn# %3s sale %6s buy %6s net %6s companies %s\n", year, month, NR, int(tsale), int(tbuy), int(tbuy-tsale), company); } }' 
   done
done
fi
