...
# only schema
mysqldump --no-data -uroot -proot ${DB_NAME} > ${DB_SCHEMA_SQL}

# full db
mysqldump -uroot -proot ${DB_NAME} > ${DB_DUMP_SQL}

# compat : pgsql
mysqldump -uroot -proot --compatible=postgresql --default-character-set=utf8 ${DB_NAME} > ${PG_DB_DUMP_SQL}
